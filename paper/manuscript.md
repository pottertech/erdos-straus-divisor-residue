...
**Status:** The converse is proven unconditionally. The forward direction is computationally supported (0 mismatches, 10,096 cases) and proven for the h = 2 and order-2 QNR sub-cases. The original −1 route does not establish the general case, because −1 ∈ H(A) does not always imply −1 ∈ D_A (821 counterexamples). Some successful cases reach T ∈ D_A by alternative divisor paths. A complete proof requires a direct argument — either through the centered divisor-residue set C_A(N), or by finding a direct construction that bypasses the −1 factorization. ∎

### 3.3 The Partial −1 Route

**Partial −1 Route Result.** *For prime A and gcd(n, A) = 1, the implication −1 ∈ H(A) ⟹ −1 ∈ D_A((nx)²) holds in the following proven subcases:*

1. *h = |H(A)| = 2*
2. *H(A) contains an order-2 generator p ≡ −1 (mod A) among the prime factors of nx*

*Outside these subcases, the implication is not valid in general. In the tested range, 821 cases have −1 ∈ H(A) but −1 ∉ D_A((nx)²). Nevertheless, the target T may still lie in D_A through alternative divisor paths (see §3.6 and analysis/item1_lemma/ for centered-residue computational groundwork).*

**Proof of the proven subcases.** Let h = |H(A)|, the order of the subgroup generated by the prime factors of nx modulo A. Since −1 ∈ H(A), h is even. Let g be a primitive root mod A, and let u = g^{(A−1)/h} be a generator of H(A). Since −1 ∈ H(A) and h is even, −1 = u^{h/2} (the unique element of order 2). For each prime factor p_i ∣ nx, write p_i ≡ u^{s_i} (mod A) where s_i = dlog_u(p_i).
...
We consider the proven subcases.
...
| Order-2 QNR (h > 2) | 1,919 | ✅ Yes | p¹ = u^{h/2} = −1 directly |
| Kneser (trivial additive stabilizer in Z/mZ, Σ(2eᵢ+1) ≥ m+k−1) | 0 | ⚠️ Under review | No cases pass with true discrete-log computation |
| Size-only (non-trivial additive stabilizer) | 4,608 | ⚠️ Candidate | Size condition met; stabilizer non-trivial |
...
**Corollary (Sufficient Condition, conditional on Theorem 3 forward direction).** *If (A/n) = −1 (i.e., A is a quadratic non-residue mod n), then A works (conditional on the forward direction of Theorem 3; computationally verified in all tested cases).*
...
**Theorem 5 (Legendre Symbol Identity).** *Let n be prime with n ≡ 1 (mod 4), let A be an odd prime with A ≡ 3 (mod 4), and let p be an odd prime divisor of m = (n+A)/4 with p ∤ A. Then the Legendre symbol (p/A) equals (n/p). That is, p is a quadratic non-residue mod A if and only if n is a quadratic non-residue mod p.*
...
**Conditional Corollary.** Assuming the forward direction of Theorem 3, A works if (A/n) = −1 (the **direct route**) OR some prime p | (n+A)/4 has (n/p) = −1 (the **m-route**). Both routes are governed by the same quadratic character of n.
...
1. **Direct T ∈ D_A criterion:** The original −1-route (−1 ∈ H(A) ⟹ −1 ∈ D_A) fails in 821 tested cases and should not be treated as a general lemma. The correct open problem is to prove a direct criterion for T ∈ D_A, likely through the centered divisor-residue set C_A(N) = {∏ p_i^{δ_i} mod A : −e_i ≤ δ_i ≤ e_i}. The centered criterion h/2 ∈ Σ_A(N) is perfectly equivalent to T ∈ D_A across all 14,244 tested cases with zero mismatches.

2. **Centered divisor-residue set and D_A closure:** The clean formulation of the closure problem uses the **centered divisor-residue set**. Let N = nx = nm with factorization N = ∏ p_i^{e_i}. A divisor P ∣ N² has P = ∏ p_i^{α_i} with 0 ≤ α_i ≤ 2e_i. The target condition P ≡ −N (mod A) is equivalent to PN⁻¹ ≡ −1 (mod A), where PN⁻¹ = ∏ p_i^{α_i − e_i}. Define the centered set C_A(N) = {∏ p_i^{δ_i} mod A : −e_i ≤ δ_i ≤ e_i}. Then A works ⟺ −1 ∈ C_A(N). **Open problem:** prove that when some prime factor of N is a QNR modulo prime A, the centered set C_A(N) contains −1. This symmetric formulation directly captures the exponent budget and is cleaner than the shifted-set approach.
